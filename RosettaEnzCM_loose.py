...
#        q.close()
        b = a.replace(",", " ")
        c = b.split(" ")
        list1 = c # Strips spaces and commas from input file. #
        ...
        temp_res[list1[0]] = list2 # Generates dictionary with template and one-letter-code residue info. #

template = []
positions = []
temp_pos = {}

for pdbfile in os.listdir('templates/'):
    if pdbfile.endswith('.pdb'):
        template.append(pdbfile[:-4])
...
#    template.append(i)
    ...
    for j in temp_res[i]:
        ...
        #######  TIM to loosen constraint on matches
        occurcount = 0
        entrycounter = 0
        df_forj = df1[df1[i] == j].iloc[0]
        for item in df1[df1[i] == j].iloc[0].tolist():
            item1 = item[0]
            entrycounter += 1
            if item1 == j[0]:
                occurcount += 1
            if item1 == '-':
                entrycounter += 1000
        howmany = occurcount / entrycounter 
        if howmany > 0.5:
            if df1[df1[i] == j].iloc[0].name in positions:
                pass
            else:
                print(j+' appears '+str(occurcount)+'times so it will count')
                with open(i+'.data' , 'a') as q:
                    q.write(seq3(j[0]).upper()+j[1:]+' ')
                q.close
                for pdbentry in template:
                    if pdbentry == i:
                        pass
                    else:
                        with open(pdbentry+'.data' , 'a') as q:
                            restowrite = df_forj[pdbentry]
                            q.write(seq3(restowrite[0]).upper() + restowrite[1:] + ' ')
                positions.append(df1[df1[i] == j].iloc[0].name)
        ####### TIM end
        else:
            print(j+' appears '+str(occurcount)+'times, so it will NOT count')
    temp_pos[i] = posit
        ...
        with open(i, 'r') as f:
            for line in f:
                ...
                for j in range(len(list1)):
                    ...
                    restype_list.append(res[:3])                                      #######
        pose = pyrosetta.rosetta.core.import_pose.pose_from_file('templates/%s.pdb' % (template_n))    
        for k in range(0,len(res_list)): # Now we will begin calculating atomic distances. #
            pairing.append(k+1)
            if k <= len(res_list)-2:
                x = res_list[k]
                y = res_list[k+1]
                xtype = restype_list[k]                                      #######
                ytype = restype_list[k+1]                                      #######
            else:
                x = res_list[0]
                y = res_list[len(res_list)-1]    
                xtype = restype_list[0]                                      #######
                ytype = restype_list[len(restype_list)-1]                                      #######
            if xtype == 'GLY' or xtype == 'ALA' or ytype == 'GLY' or ytype == 'ALA':     ########
                residue1.append(x)                                      #######
                residue2.append(y)                                      #######
                CA1CA2.append(None)                                      #######
                CB1CB2.append(None)                                      #######
                CA1CB2.append(None)                                      #######
                CB1CA2.append(None)                                      #######
                FullTemp.append(template_n)                                      #######
            else:                                       ###### blocks below indented for gly test#
...
for i in df_fam['ID']: # Slices new DF for each target. #
    if i in template:
        ...
        print('Catalytic constraints for target: ' + str(i))
        with open('%s.dist_csts' % (i), 'w') as h: # Writes catalytic constraints to file. #
            ...
            for j in pairs:
                if j <= pairs[-2]: # Generates residue pairs based upon number of catalytic residues. #
                    res1 = str(df_famtest['Pos%s' % (j)].iloc[0])
                    res2 = str(df_famtest['Pos%s' % (j+1)].iloc[0])
                else: 
                    res1 = str(df_famtest['Pos%s' % (pairs[0])].iloc[0])
                    res2 = str(df_famtest['Pos%s' % (pairs[-1])].iloc[0])
